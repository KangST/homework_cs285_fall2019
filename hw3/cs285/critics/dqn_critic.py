...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            q_tp1_values_ = q_func(self.obs_tp1_ph, self.ac_dim, scope='q_func', reuse=True)
            idx = tf.one_hot(tf.argmax(q_tp1_values_, axis=-1), self.ac_dim)
            q_tp1 = tf.reduce_sum(self.q_tp1_values * idx, axis=1)
        else:
